...
class BinarySearchTree {
  ...
  levelOrder(root: Node | null) {
    if (!root) return;

    const levels: Record<string, Node[]> = {};

    const getLevels = (node: Node, depth: number) => {
      if (!levels[depth]) {
        levels[depth] = [];
      }

      levels[depth].push(node);
      if (node.left) getLevels(node.left, depth + 1);
      if (node.right) getLevels(node.right, depth + 1);
    };
    getLevels(root, 0);

    for (let nodes of Object.values(levels)) {
      nodes.forEach((n) => process.stdout.write(`${n.data} `));
    }
  }
}
